...
const checkRegister = () => {
    ...
    let totalCid = cid.reduce((total, sum) => total + sum[1], 0).toFixed(2); 
    ...
    if (change > totalCid) {
        displayChangeDue.innerHTML = "Status: INSUFFICENT_FUNDS";
        return;
    }

const denominations = [100, 20, 10, 5, 1, 0.25, 0.10, 0.05, 0.01];
const denominationsNames = ["ONE HUNDRED", "TWENTY", "TEN", "FIVE", "ONE", "QUARTER", "DIME", "NICKEL", "PENNY"];
let changeArr = [];
let cidCopy = [...cid];
...
let remainingCid = cidCipy.reduce((total, sum) => total + sum[1], 0);
if (remainingCid === 0) {
